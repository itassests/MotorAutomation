...
async function processOnePolicy(pool, policy, marginRules, caches, statementIndex, prIndex, specialRulesByAgent, globalUpliftByAgent) {
  ...
  // ---- IndusInd CV / PCV / GCV / MISC — "June CV26" grid (USER 2026-06-26) ----
  // 72 region × ~25 segments. config/indusind_cv.json + services/indusind-cv.js
  // (region from City + RTO state; segment from vehicle type/tonnage/seat/body).
  if (insurerSlug === 'indusind' &&
      /PCV|GCV|MISC|MIS/.test(String(params.vehicleType || '').toUpperCase())) {
    try {
      const { resolveIndusindCvRate } = require('../services/indusind-cv');
      const ir = resolveIndusindCvRate(params);
      if (ir != null) {
        const _b = rules[0];
        const _seg = 'CV/PCV (IndusInd Jun26 grid)';
        rules = [_b
          ? { ..._b, rate_value: ir, segment: _seg }
          : { id: -1, insurer: 'indusind', product: String(params.vehicleType || 'PCV').toUpperCase(),
              region: resolvedRegion || '', rate_type: (Number(params.odPremium) || 0) <= 0 ? 'SATP' : 'COMP',
              rate_value: ir, segment: _seg, is_declined: 0 }];
      }
    } catch (_) { /* leave rules unchanged on any failure */ }
    ...
    try {
      ...
      if (reg) {
        ...
        if (is3W) segClause = isElectric ? "segment LIKE '%GCCV_3W%Electric%'" : "segment LIKE '%GCCV_3W%'";
          ...
          // ≤3.5T GCV splits by the make-specific "TATA/ Maruti/ Mahindra" row
          // (2_UPTO_3.5T, GJ 0.50) vs "All Other Make/Models" (GJ 0.58 / MH 0.45).
          // The make row is really a MODEL list — the grid names them: Ace / Super Ace,
          // Yodha, Xenon, Magic, Intra, Super Carry, Jeeto, Supro. Only those models
          // take 0.50; every OTHER model (incl. Mahindra Bolero / Bolero Maxi Truck,
          // Tata LPT, AL Dost) → "Other" 0.58 — which is what the operator pays for a
          // Bolero. segment IS NOT NULL drops the null-segment artifact (MH 0.55).
          // ELECTRIC goods carriers (Euler / E-rickshaw) use the "[Electric]" variant.
          const isMakeSectionModel = /\bACE\b|YODHA|XENON|MAGIC|INTRA|CARRY|JEETO|SUPRO/i.test(String(params.model || ''));
          if (isElectric) segClause = "segment LIKE '%UPTO[_]3.5%Electric%'";
          else segClause = isMakeSectionModel
